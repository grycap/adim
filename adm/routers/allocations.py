...
from adm.authorization import authenticate
from adm.models.allocation import AllocationInfo, Allocation, AllocationId
from adm.models.page import PageOfAllocations
from adm.models.success import Success
from adm.utils.node_registry import EOSCNodeRegistry
...
def list_allocations(
    ...
    try:
        adm.logger.debug(f"Listing allocations from user '{user_info.get('sub')}'")
        count, allocations = adm.allocation_store.list_allocations(user_info, from_, limit)
    ...
    if all_nodes:
        adm.logger.debug(f"Listing allocations in all EOSC nodes for user '{user_info.get('sub')}'")
...
def _get_allocation_info(allocation_id: str, user_info: dict, request: Request) -> AllocationInfo:
    try:
        allocation_data = adm.allocation_store.get_allocation(allocation_id, user_info)
...
def get_allocation(request: Request,
    ...
    adm.logger.debug(f"Getting allocation {allocation_id} from user '{user_info.get('sub')}'")
...
def _check_allocation_in_use(allocation_id: str, user_info: dict) -> Response | None:
    ...
    try:
        _, deployments = adm.deployments_manager.list_deployments(limit=999999999, user_info=user_info)
...
def update_allocation(allocation_id,
                      ...
                      request: Request,
                      user_info=Depends(authenticate)):
    adm.logger.debug(f"Update allocation {allocation_id} from user '{user_info.get('sub')}'")
    allocation_info = _get_allocation_info(allocation_id, user_info, request)
    if allocation_info is None:
        return return_error("Allocation not found", status_code=404)
    ...
    try:
        adm.allocation_store.replace_allocation(data, user_info, allocation_id)
    except Exception as ex:
        return return_error(str(ex), 503)
...
def delete_allocation(allocation_id,
    ...
    adm.logger.debug(f"Delete allocation {allocation_id} from user '{user_info.get('sub')}'")
    if not adm.allocation_store.get_allocation(allocation_id, user_info):
        return return_error("Allocation not found", status_code=404)
    ...
    try:
        adm.allocation_store.delete_allocation(allocation_id, user_info)
    except ConnectionException as ex:
        return return_error(str(ex), 503)
...
def create_allocation(allocation: Allocation,
    ...
    adm.logger.debug(f"Create allocation from user '{user_info.get('sub')}'")
    data = allocation.model_dump(exclude_unset=True, mode="json")
    adm.logger.debug(f"Allocation data: {allocation.model_dump()}")

    found_id = adm.allocation_store.check_allocation_exists(data, user_info)
    if found_id:
    ...
    try:
        allocation_id = adm.allocation_store.replace_allocation(data, user_info)
    except Exception as ex:
        return return_error(str(ex), 503)
