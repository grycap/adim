...
from adm.models.page import PageOfTools
...
def list_tools(
    ...
    try:
        adm.logger.debug(f"Listing tools from user '{user_info.get('sub')}'")
        total, count, tools = adm.tool_store.list_tools(request, from_, limit, user_info)
...
def get_tool(tool_id: str,
    ...
    try:
        adm.logger.debug(f"Getting tool {tool_id} from user '{user_info.get('sub')}'")
        tool_or_msg, status_code = adm.tool_store.get_tool(tool_id, version, request)
    except ConnectionException as ex:
        return return_error("Repository connection failed: %s" % ex, 503)
