...
from adm.models.deployment import DeploymentInfo, DeploymentId, Deployment, CloudQuota
from adm.models.page import PageOfDeployments
from adm.utils.node_registry import EOSCNodeRegistry
from adm.utils import DBConnectionException
...
def _list_deployments(user_info: dict, request: Request, from_: int = 0,
    ...
    try:
        count, deployments = adm.deployments_manager.list_deployments(user_info, from_, limit)
...
def list_deployments(
    ...
    adm.logger.debug(f"Listing deployments from user '{user_info.get('sub')}'")
...
def get_deployment(deployment_id,
    ...
    adm.logger.debug(f"Getting deployment {deployment_id} from user '{user_info.get('sub')}'")
    deployment, status_code = adm.deployments_manager.get_deployment(deployment_id, user_info, True)
...
def delete_deployment(deployment_id,
    ...
    adm.logger.debug(f"Deleting deployment {deployment_id} from user '{user_info.get('sub')}'")
    res, status_code = adm.deployments_manager.delete_deployment(deployment_id, user_info)
...
def deploy_workload(deployment: Deployment,
    ...
    adm.logger.debug(f"Creating deployment from user '{user_info.get('sub')}'")
    # Get the Tool from the ID
    tool, status_code = adm.tool_store.get_tool(deployment.tool.id, deployment.tool.version, request)
    if status_code != 200:
        adm.logger.warning(f"Tool {deployment.tool.id} not found")
        return Response(content=tool, status_code=400, media_type="application/json")

    # Get the allocation info from the Allocation
    allocation_info, status = adm.deployments_manager.get_allocation(deployment, user_info)
    if status != 200:
        adm.logger.warning(f"Allocation {deployment.allocation.id} not found")
        return allocation_info, status

    if allocation_info.allocation.root.kind == "EoscNodeEnvironment":
        adm.logger.error("EOSCNodeEnvironment support not implemented yet")
        raise NotImplementedError("EOSCNodeEnvironment support not implemented yet")

    try:
        deployment_info = adm.deployments_manager.update_deployment(deployment, tool, allocation_info,
