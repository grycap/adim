...
from adm.models.deployment import DeploymentInfo, Deployment, CloudQuota
from adm.models.tool import ToolInfo
from adm.models.error import Error
from adm.models.success import Success
from adm.models.allocation import Allocation, AllocationInfo
from typing import Tuple, Union
from adm.utils.db import DataBase
from adm.utils import ConnectionException, DBConnectionException
...
class DeploymentsManager:
    ...
    def _init_table(db: DataBase) -> bool:
        ...
        if not db.table_exists("deployments"):
            adm.logger.info("Creating deployments table")
            ...
            if self.db.db_type == DataBase.MONGO:
                ...
                for count, elem in enumerate(res):
                    ...
                    try:
                        ...
                        adm.logger.error("Failed to parse deployment info from database: %s", str(ex))
                        continue
    ...
    def get_allocation(self, deployment: Deployment, user_info: dict) -> Tuple[Union[Error, AllocationInfo], int]:
        ...
        try:
            allocation_data = adm.allocation_store.get_allocation(deployment.allocation.id, user_info)
        except ConnectionException as ex:
            adm.logger.error(f"Error connecting to Allocation Store: {str(ex)}")
    ...
    def _get_state(self, dep_info: DeploymentInfo, user_info: dict) -> DeploymentInfo:
        try:
            ...
            if success:
                ...
                adm.logger.error(f"Could not retrieve deployment status: {state_info}")
                # Check if the infrastructure still exists
                success, infras = client.list_infras()
                if success:
                    if dep_info.id not in infras:
                        adm.logger.info(f"Deployment {dep_info.id} not found in IM."
                                        " Setting status to 'deleted'")
                        dep_info.status = "deleted"
                else:
                    adm.logger.error(f"Could not list infrastructures: {infras}")

            success, outputs = client.get_infra_property(dep_info.id, "outputs")
            if success:
                dep_info.outputs = outputs
            else:
                adm.logger.error(f"Could not get deployment outputs: {outputs}")

            success, cont_msg = client.get_infra_property(dep_info.id, "contmsg")
            if success:
                dep_info.details = cont_msg
            else:
                adm.logger.error(f"Could not get deployment contmsg: {cont_msg}")
        except Exception as ex:
            adm.logger.error(f"Error retrieving deployment state: {str(ex)}")
        ...
        if self.db.connect():
            ...
            if res:
                ...
                try:
                    ...
                    adm.logger.error(f"Failed to parse deployment info from database: {str(ex)}")
    ...
    def _get_template(blueprint: str, inputs: Dict[str, Any]) -> str:
        if not inputs:
            return blueprint
        adm.logger.debug(f"Input values: {inputs}")
    ...
    def update_deployment(self, deployment: Deployment, tool: ToolInfo,
        ...
        if dry_run:
            ...
            if not success:
                adm.logger.error("Could not get cloud quotas: %s", quotas)
                quotas = {}
            ...
            if self.db.connect():
                ...
                adm.logger.debug(f"Storing deployment info: {data}")
                if self.db.db_type == DataBase.MONGO:
    ...
    def get_deployments_manager():
        ...
        db_url = os.getenv("DB_URL", "file:///tmp/adm.db")
