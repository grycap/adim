...
class AllocationStore():
    ...
    def get_allocation_store() -> 'AllocationStore':
        ...
        if store_type == "db":
            from adm.utils.allocation.allocation_store_db import AllocationStoreDB
            db_url = os.getenv("DB_URL", AllocationStoreDB.DEFAULT_URL)
            allocation_store = AllocationStoreDB(db_url)
            adm.logger.info(f"Using AllocationStoreDB with URL: {db_url}")
            ...
            adm.logger.info(f"Using AllocationStoreVault with URL: {vault_url}")
        else:
