...
class ToolStore:
    ...
    def list_tools(self, request: Request, from_: int = 0, limit: int = 100,
        ...
        for tool in tools_list:
            ...
            try:
                ...
                adm.logger.error("Failed to get tool info: %s", ex)
    ...
    def get_tool_store() -> 'ToolStore':
        ...
        if tool_type == "git":
            tools_repo = os.getenv("ADM_TOOLS_REPO", "https://github.com/grycap/tosca/blob/eosc_lot1/templates/")
            from adm.utils.tool.git_tool_store import ToolStoreGit
            tool_store = ToolStoreGit(tools_repo)
        elif tool_type == "rc":
            resource_catalog = os.getenv("RESOURCE_CATALOG", "https://providers.sandbox.eosc-beyond.eu/api")
            from adm.utils.tool.rc_tool_store import ToolStoreRC
