...
from adm.utils.deployment_manager import DeploymentsManager
...
def db_mock(mocker):
    ...
    db = mocker.patch("adm.utils.deployment_manager.DataBase", return_value=instance)
...
def seed_deployments(db_mock):
    def _seed(deployments):
        adm.deployments_manager = DeploymentsManager("", "")
        adm.deployments_manager.db = db_mock
...
def check_oidc_mock(mocker):
    ...
    mocked = mocker.patch("adm.authorization.check_OIDC")
...
def im_mock(mocker):
    """Mock para IMClient."""
    mocked = mocker.patch("adm.utils.deployment_manager.IMClient.init_client")
    im_mocked = MagicMock()
...
def allocation_mock(mocker):
    ...
    return mocker.patch("adm.allocation_store.get_allocation", return_value=allocation)
...
topology_template:
  inputs:
    num_cpus:
      ...
      default: 2
    """
    return mocker.patch("adm.tool_store.get_tool", return_value=(tool, 200))
