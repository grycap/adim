...
def db_mock(mocker):
    ...
    db = mocker.patch("adim.utils.deployment_manager.DataBase", return_value=instance)
...
def seed_deployments(db_mock):
    def _seed(deployments):
        adim.deployments_manager = DeploymentsManager("", "")
        adim.deployments_manager.db = db_mock
...
def check_oidc_mock(mocker):
    ...
    mocked = mocker.patch("adim.authorization.check_OIDC")
...
def im_mock(mocker):
    """Mock para IMClient."""
    mocked = mocker.patch("adim.utils.deployment_manager.IMClient.init_client")
    im_mocked = MagicMock()
...
def allocation_mock(mocker):
    ...
    return mocker.patch("adim.allocation_store.get_allocation", return_value=allocation)
...
def test_get_deployment(client, db_mock, check_oidc_mock, im_mock, allocation_mock, seed_deployments):
    ...
    assert response.json()["tool"]["id"] == "toolid"
...
def test_deploy_workload(
    ...
    payload = ('{"tool": {"kind": "ToolId", "id": "toolid", "infoLink": "http://some.url/"}, '
               '"allocation": {"kind": "AllocationId", "id": "aid", "infoLink": "http://some.url/"}}')
