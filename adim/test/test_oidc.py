...
import pytest
import adim.authorization
from adim.oidc.client import OpenIDClient
from adim.authorization import check_OIDC
...
def jwt_mock(mocker):
    ...
    return mocker.patch("adim.oidc.client.JWT.get_info")


@pytest.fixture
def requests_mock(mocker):
    """Mock para requests.request()"""
    return mocker.patch("adim.oidc.client.requests.request")


@pytest.fixture
def time_mock(mocker):
    """Mock para time.time()"""
    return mocker.patch("adim.oidc.client.time.time")
...
def test_get_token_introspection_success(requests_mock, token):
    ...
    OpenIDClient.ISSUER_CONFIG_CACHE = {}
    adim.authorization.OIDC_ISSUERS = ["https://issuer.example.com"]
...
def test_auth_check_oidc_issuers(requests_mock, jwt_mock, time_mock, token):
    ...
    adim.authorization.OIDC_ISSUERS = ["https://other-issuer.example.com"]
    with pytest.raises(HTTPException):
        check_OIDC(token)
...
def test_auth_check_oidc_aud(requests_mock, jwt_mock, time_mock, token):
    ...
    OpenIDClient.ISSUER_CONFIG_CACHE = {}
    adim.authorization.OIDC_ISSUERS = ["https://issuer.example.com"]
    adim.authorization.OIDC_AUDIENCE = "adim"
    res = check_OIDC(token)
    assert res["sub"] == "user123"
    ...
    adim.authorization.OIDC_AUDIENCE = ""
...
def test_auth_check_oidc_groups(requests_mock, jwt_mock, time_mock, token):
    ...
    OpenIDClient.ISSUER_CONFIG_CACHE = {}
    adim.authorization.OIDC_ISSUERS = ["https://issuer.example.com"]
    adim.authorization.OIDC_GROUPS = "adim"
    res = check_OIDC(token)
    assert res["sub"] == "user123"
    ...
    adim.authorization.OIDC_GROUPS = ""
