...
def db_mock(mocker):
    ...
    db = mocker.patch("adim.utils.allocation.allocation_store_db.DataBase", return_value=instance)
...
def list_deployments_mock(mocker):
    list_dep_mock = mocker.patch("adim.deployments_manager.list_deployments")
...
def seed_allocations(backend_type, db_mock, vault_mock, monkeypatch):
    def _seed(allocations_list):
        ...
        from adim.utils.allocation.allocation_store import AllocationStore
        adim.allocation_store = AllocationStore.get_allocation_store()

        # Setup mocks based on backend type
        if backend_type in ["db", "mongo"]:
            adim.allocation_store.db = db_mock
